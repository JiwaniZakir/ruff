                        ...
                        if dataclass_field_specifiers.contains(&function) =>
                    {
                        ...
                        let get_metadata_argument_type =
                            |name, fallback_to_default| -> Option<Type<'db>> {
                                ...
                                // If an overload accepts them through `**kwargs`, the bound
                                // parameter type erases literal values like `init=False`,
                                // `kw_only=True`, or `alias="name"`, so prefer the call-site type.
