...
fn check_class_declaration<'db>(
    ...
    let mut overridden_superclass_member: Option<(ClassType<'db>, Option<Definition<'db>>)> = None;
    ...
    if !is_private_member {
        for class_base in class.iter_mro(db).skip(1) {
            ...
            let is_synthesized_superclass_member = if let Some(id) = superclass_symbol_id {
            ...
            if configuration.check_missing_overrides() && overridden_superclass_member.is_none() {
                // `@override` is required for user-authored concrete superclass behavior, but not
                // for interpreter hooks, ty-generated fallback members, or interface-only targets.
                if !is_synthesized_superclass_member
                    && !superclass.is_object(db)
                    && !superclass.is_known(db, KnownClass::Type)
                {
                    let superclass_definition = superclass_symbol_id
                        .and_then(|id| symbol_definition(db, superclass_scope, id));
                    if !is_interface_override_target(db, superclass, superclass_type) {
                        overridden_superclass_member = Some((superclass, superclass_definition));
                    }
                }
            }
    ...
    if let Some((superclass, superclass_definition)) = overridden_superclass_member
        && configuration.check_missing_overrides()
    ...
    {
        check_missing_overrides(context, member, superclass, superclass_definition);
    }
...
fn check_missing_overrides<'db>(
    ...
    let Some(missing_override_definition) =
        missing_override_decorator_definition(db, member.ty, context.in_stub())
    else {
        return;
    };
    ...
    let Some(builder) = context.report_lint(
        ...
        missing_override_definition.focus_range(db, context.module()),
    ) else {
        return;
    };
...
fn is_interface_override_target<'db>(
    db: &'db dyn Db,
    superclass: ClassType<'db>,
    superclass_type: Type<'db>,
) -> bool {
    let Some(functions) = extract_underlying_functions(db, superclass_type) else {
        return false;
    };

    if superclass.is_protocol(db) {
        return functions.iter().all(|function| {
            function.definition(db).file(db).is_stub(db)
                || function.as_abstract_method(db, superclass).is_some()
        });
    }
    ...
    functions
        .iter()
        .all(|function| function.as_abstract_method(db, superclass).is_some())
}

fn missing_override_decorator_definition<'db>(
    db: &'db dyn Db,
    ty: Type<'db>,
    in_stub: bool,
    ...
    extract_underlying_functions(db, ty)?
        .into_iter()
        .find_map(|function| {
            let definition = override_decorator_definition(db, function, in_stub);
            (!definition.has_known_decorator(db, FunctionDecorators::OVERRIDE))
                .then_some(definition)
        })
}

fn override_decorator_definition<'db>(
    db: &'db dyn Db,
