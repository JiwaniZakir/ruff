...
struct Relevance {
    ...
    /// rank unrelated matches before full origin breaks same-label ties.
...
impl Relevance {
    ...
    fn new(_ctx: &CollectionContext, query: &UserQuery, c: &CompletionBuilder) -> Relevance {
        Relevance {
            ...
            match_quality: if c.is_type_check_only || c.deprecated {
                MatchQuality::Fuzzy
            } else {
                query.match_quality(&c.name)
            },
        }
    }
}
...
///
/// The variant order is the completion ranking order after broader relevance
/// criteria: earlier origins rank higher.
    ...
    fn add_completions<'db>(
        ...
        match *self {
            ImportStatement::Import(Import { ref kind, .. }) => match *kind {
                ImportKind::Module => {
                    add_import_semantic_completions(db, completions, model.import_completions());
                }
                ImportKind::Submodule { ref parent } => {
                    add_import_semantic_completions(
                        db,
                        completions,
            ...
            ImportStatement::FromImport(FromImport { ast, ref kind }) => match *kind {
                ...
                FromImportKind::Attribute => {
                    ...
                    add_import_semantic_completions_with_module_dependency_kind(
                        db,
                        completions,
...
fn add_import_semantic_completions_with_module_dependency_kind<'db>(
    db: &'db dyn Db,
    completions: &mut Completions<'db>,
...
fn add_import_semantic_completions<'db>(
    db: &'db dyn Db,
    completions: &mut Completions<'db>,
    semantic_completions: impl IntoIterator<Item = SemanticCompletion<'db>>,
) {
    for semantic in semantic_completions {
        let module_dependency_kind = if let Some(Type::ModuleLiteral(module_literal)) = semantic.ty
        {
...
impl Ord for CompletionRanker<'_> {
    fn cmp(&self, rhs: &CompletionRanker<'_>) -> Ordering {
        (&self.0.relevance, &self.0.name)
            .cmp(&(&rhs.0.relevance, &rhs.0.name))
            // Prefer stdlib over third-party modules only for equally named
            // completions; unrelated labels should keep their name ordering.
            .then_with(|| {
                self.0
                    .module_dependency_kind
                    .cmp(&rhs.0.module_dependency_kind)
            })
            .then_with(|| self.0.module_name.cmp(&rhs.0.module_name))
    }
}
