...
impl Db {
    pub(crate) fn setup() -> Self {
        ...
        db.settings = Some(Settings::new(&db, mdtest_rule_selection(None)));
        db
    }
...
fn mdtest_rule_selection(rules: Option<&Rules>) -> RuleSelection {
    ...
    // Most mdtests run with ignored-by-default rules enabled so their behavior is covered
    // alongside the default rules. `missing-override-decorator` is different: enabling it across
    // the entire suite turns unrelated fixtures into style-policy tests, so keep it opt-in.
